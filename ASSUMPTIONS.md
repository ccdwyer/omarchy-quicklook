...
- **Prebuilt x86_64/aarch64 musl binaries are not in this git tree.** This host is macOS and has no linux-musl toolchain. The required distributable path is `.github/workflows/release-helper.yml` (cross-compile + checksums on the GitHub release) plus `scripts/fetch-helper.sh` and `build.sh`. Cold-judge install does not need them: `compat/` + the demo corpus satisfy `omarchy plugin add --enable`. We do not claim binaries exist in the repo.
