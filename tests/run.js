...
test("binds: plan exposes hotkeyLabel and canSet, never reserved combos", () => {
  const empty = Binds.plan([])
  assert.strictEqual(empty.canSet, true)
  assert.strictEqual(empty.hotkeyLabel, "")
  assert.ok(Binds.luaBlock(empty.toAdd).indexOf("hl.unbind") < 0)
  assert.ok(Binds.CANDIDATES.every((c) => c.keys !== "SUPER + SHIFT + P"))
  const live = [
    { modmask: 64, key: "PERIOD", dispatcher: "__lua", arg: "15", description: "QuickLook" }
  ]
  const ours = Binds.plan(live)
  assert.strictEqual(ours.needed, false)
  assert.strictEqual(ours.canSet, false)
  assert.strictEqual(ours.hotkeyLabel, "Super+.")
  assert.strictEqual(Binds.prettyCombo("SUPER + ALT + PERIOD"), "Super+Alt+.")
})

test("qml: opt-in Set hotkey on the bar, no first-load auto-assign", () => {
  ...
  assert.ok(bar.indexOf("Remove") >= 0)
  assert.ok(bar.indexOf("toggleOverlay") >= 0)
  assert.ok(bar.indexOf('"shell", "toggle"') >= 0)
  assert.ok(service.indexOf("function removeBinds") >= 0)
...
test("no unbounded subprocesses on untrusted files; json escape; cache rejects empty", () => {
  ...
  assert.ok(sh.indexOf("setsid gio open") >= 0 || sh.indexOf("setsid xdg-open") >= 0)
  ...
  assert.ok(/subprocess\.Popen\(\s*\[opener/.test(py))
  const openRs = fs.readFileSync(path.join(ROOT, "src/quicklookd/src/open.rs"), "utf8")
  assert.ok(!/run_limited\(/.test(openRs))
  assert.ok(openRs.indexOf("spawn_detached") >= 0)
  assert.ok(openRs.indexOf("setsid") >= 0)
  const svc = fs.readFileSync(path.join(ROOT, "Service.qml"), "utf8")
  assert.ok(svc.indexOf("execDetached") >= 0)
  assert.ok(svc.indexOf("xdg-open") >= 0)
...
test("manifest: id, kinds, entryPoints", () => {
  ...
  assert.deepStrictEqual(man.kinds, ["overlay", "service", "bar-widget"])
  ...
  assert.strictEqual(man.barWidget.displayName, "QuickLook")
  assert.strictEqual(man.barWidget.defaultSection, "right")
  assert.ok(fs.existsSync(path.join(ROOT, "BarWidget.qml")))
...
test("install-binds.py writes and removes only the marked block", () => {
  const { spawnSync } = require("child_process")
  const os = require("os")
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "quicklook-binds-"))
  const hypr = path.join(tmp, "hypr")
  fs.mkdirSync(hypr)
  const file = path.join(hypr, "bindings.lua")
  fs.writeFileSync(file, "-- user binds\no.bind(\"SUPER + B\", \"Browser\", { launch = \"chromium\" })\n")
  const script = path.join(ROOT, "compat/install-binds.py")
  const env = Object.assign({}, process.env, { XDG_CONFIG_HOME: tmp })
  const lua = 'o.bind("SUPER + PERIOD", "QuickLook", "omarchy-shell shell toggle io.github.chris.quicklook \'{}\'")'
  const inst = spawnSync("python3", [script, "io.github.chris.quicklook", lua], { encoding: "utf8", env })
  assert.strictEqual(inst.status, 0, inst.stderr)
  const afterInstall = fs.readFileSync(file, "utf8")
  assert.ok(afterInstall.indexOf("-- BEGIN io.github.chris.quicklook") >= 0)
  assert.ok(afterInstall.indexOf("SUPER + PERIOD") >= 0)
  assert.ok(afterInstall.indexOf("Browser") >= 0)
  assert.ok(afterInstall.indexOf("hl.unbind") < 0)
  const rm = spawnSync("python3", [script, "--remove", "io.github.chris.quicklook"], { encoding: "utf8", env })
  assert.strictEqual(rm.status, 0, rm.stderr)
  const afterRemove = fs.readFileSync(file, "utf8")
  assert.ok(afterRemove.indexOf("-- BEGIN io.github.chris.quicklook") < 0)
  assert.ok(afterRemove.indexOf("QuickLook") < 0)
  assert.ok(afterRemove.indexOf("Browser") >= 0)
  fs.rmSync(tmp, { recursive: true, force: true })
})

test("readme: opt-in bind from the bar; Remove strips bindings.lua", () => {
  const readme = fs.readFileSync(path.join(ROOT, "README.md"), "utf8")
  assert.ok(readme.indexOf("auto-assign on first load") < 0)
  assert.ok(readme.indexOf("Set hotkey") >= 0)
  assert.ok(readme.indexOf("## Remove") >= 0)
  assert.ok(readme.indexOf("omarchy plugin remove io.github.chris.quicklook") >= 0)
  assert.ok(readme.indexOf("install-binds.py --remove") >= 0)
  assert.ok(readme.indexOf("bindings.lua") >= 0)
  assert.ok(readme.indexOf("pacman -S poppler") >= 0)
  assert.ok(readme.indexOf("pacman -S plocate") >= 0)
  assert.ok(readme.indexOf("Never") >= 0 || readme.indexOf("never") >= 0)
})
